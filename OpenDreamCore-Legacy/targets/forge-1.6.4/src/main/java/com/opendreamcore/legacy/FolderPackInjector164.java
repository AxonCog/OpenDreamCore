...
/**
 * 1.6.4 材质包注入：全部塞进 Minecraft.defaultResourcePacks（反射拿这个
 * 私有 List，refreshResources 会拿它重建整条资源栈）。
 *
 * 包一层虚拟元数据：玩家丢个纯贴图文件夹没有 mcmeta 也照常加载。
 */
public final class FolderPackInjector164 implements FolderPackInjector {
    ...
    public void injectRoot(java.nio.file.Path root) {
        ...
        try {
            ...
            stack.add(new ManagedPack164(root.toFile(), 1)); // 1.6.4 pack_format = 1
            rootInjected = true;
            mc.refreshResources();
            OdcLegacy164.LOGGER.info("[ODC] 托管根目录已挂载: {}", root);
        } catch (Throwable t) {
            if (!rootWarned) {
                rootWarned = true;
                OdcLegacy164.LOGGER.warn("[ODC] 托管根目录挂载失败: {}", t.toString());
            }
        }
    }
    ...
    public int injectAll(List<File> packs) {
        ...
        try {
            ...
            for (File f : packs) {
                try {
                    Object pack = f.isDirectory()
                            ? new MarkedFolderPack(f)
                            : new MarkedFilePack(f);
                    stack.add(pack);
                    ok++;
                    OdcLegacy164.LOGGER.info("[ODC] 材质包挂载: {}", f.getName());
                } catch (Throwable t) {
                    OdcLegacy164.LOGGER.warn("[ODC] 材质包挂载失败 {}: {}", f.getName(), t.toString());
                }
            }
            if (ok > 0) {
                mc.refreshResources();
            ...
            OdcLegacy164.LOGGER.warn("[ODC] 材质包注入异常: {}", t.toString());
            return 0;
        }
    }
    ...
    private static List defaultPacks(Minecraft mc) throws Exception {
        ...
        for (java.lang.reflect.Field f : Minecraft.class.getDeclaredFields()) {
            ...
            for (Object item : (List) value) {
                if (item instanceof net.minecraft.client.resources.ResourcePack) {
                    return (List) value;
                }
            }
        }
    ...
    /** 虚拟 pack 元数据：1.6.4 固定 format 1，描述是 String。 */
    private static MetadataSection virtualMeta(String section) {
        if ("pack".equals(section)) {
            return new PackMetadataSection("OpenDreamCore 材质包", 1);
        }
        return null;
    }
}
