...
/**
 * 1.7.10 渲染实现：GL11 + WorldRenderer（startDrawingQuads 时代）。
 * 坐标按 ScaledResolution 逻辑像素；矩阵栈 glPush/glPop。
 */
public final class Renderer1710 implements LegacyRenderer {

    private int scaledWidth;
    private int scaledHeight;

    /** 每帧注入 ScaledResolution。 */
     ...
     * 俯仰变了自己不会歪成斜条。锚在世界坐标上正对镜头。
     * 相机扣位用 RenderManager.renderPos
     * （静态、每帧随渲染视点刷新，和实体渲染同一基准）。
     * 深度测试留着（面板会被墙挡），深度写入关掉（同 z 元素靠 LEQUAL 按序叠）。
     */
    @Override
    ...
    private static void cornerFan(Tessellator tess, double cx, double cy, double rad, double baseDeg) {
        // 圆心 = 角落点向矩形内部收 rad：
    ...
    private static void gradVertex(Tessellator tess, double x, double y, double yTop, double yBot,
                                   int topArgb, int bottomArgb) {
    ...
    private static void gradQuad(Tessellator tess, double x0, double y0, double x1, double y1,
    ...
    private static void cornerFanGrad(Tessellator tess, double cx, double cy, double rad,
    ...
    public void drawImage(String texture, double x, double y, double w, double h,
        ...
        // 1.7.10 的 MCP 映射里 getResource 没声明 IOException，运行时抛的
        // FileNotFoundException 又是运行时逮不住的——这里只能 Throwable 兜底，
        // 反正只干一件事：找不到就喊一声然后不画。
        try {
