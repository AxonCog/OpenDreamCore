...
/**
 * 托管根目录常驻包（1.7.10）：resourcepacks/OpenDreamCore 本身当一张包。
 *
 ...
 */
public final class ManagedPack1710 implements IResourcePack {

    /** 托管根目录。 */
    private final File base;
    /** 虚拟元数据里报的包格式（1.7.10 = 1）。 */
    private final int packFormat;

    public ManagedPack1710(File base, int packFormat) {
        this.base = base;
    ...
    public IMetadataSection getPackMetadata(IMetadataSerializer serializer, String section) {
        if ("pack".equals(section)) {
            return new PackMetadataSection(
                    new net.minecraft.util.ChatComponentText("OpenDreamCore 托管材质包"),
                    packFormat);
        }
        return null;
    }
