...
public final class UiChannel {
    ...
    private static net.minecraft.resources.ResourceLocation channel(String path) {
        return net.minecraft.resources.ResourceLocation.fromNamespaceAndPath(Protocol.NAMESPACE, path);
    }
    ...
    public static void registerPayloads(PayloadRegistrar registrar) {
        ...
        // 双向 payload：用 commonBidirectional（NeoForge 不允许同 ID 注册两次）
        registrar.commonBidirectional(pageLayoutType(), rawCodec(pageLayoutType()), HandshakeHandler::handlePageLayout);
        registrar.commonBidirectional(editorLeaseType(), rawCodec(editorLeaseType()), HandshakeHandler::handleLease);
        registrar.commonBidirectional(customPacketType(), rawCodec(customPacketType()), HandshakeHandler::handleCustomPacket);
    }
}
