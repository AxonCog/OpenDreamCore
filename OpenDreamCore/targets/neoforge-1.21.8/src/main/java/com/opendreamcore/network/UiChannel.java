...
public final class UiChannel {
    ...
    public static void registerPayloads(PayloadRegistrar registrar) {
        ...
        registrar.commonBidirectional(chunkType(), rawCodec(chunkType()),
                HandshakeHandler::handleChunk, HandshakeHandler::handleChunk);
        ...
        // 双向 payload：用四参重载（serverHandler + clientHandler 分开传）。
        // 21.8 起三参重载只填服务端槽，客户端侧校验会报 missing client-side handlers 直接炸；
        // 同 ID 分两次注册又不允许，所以必须成对一次传齐。
