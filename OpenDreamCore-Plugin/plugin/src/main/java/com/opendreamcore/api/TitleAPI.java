...
public final class TitleAPI {
     ...
     * 完整配置推送。
     *
     * text：兜底文本（titles 为空时显示）
     * titles：轮播序列
     * typewriter：打字机逐字显现
     * random：随机轮换（多句时随机选句；优先于打字机效果）
     * speed：打字机每字符毫秒
     * interval：每句展示时长基准毫秒
     * holdMs：打完停留毫秒（-1 = 取 interval）
     * loop：是否循环
     */
    ...
    private Iterable<? extends Player> players() {
        ...
        return plugin == null ? java.util.Collections.emptyList() : plugin.getServer().getOnlinePlayers();
    }
